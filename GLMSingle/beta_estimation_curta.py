...
subjects = ["03"] # subjects 9 and 16 excluded due to motion
    ...
    for runTypeIdx in runtypes:
        ...
        for smoothing in range(2):
            ...
            for i in range(len(matches)):
                ...
                upsamled_matrix = upsampled_matrix[1:-1, :]
                print(f"Size of Design Matrix after upsampling: {upsampled_matrix.shape}")
                ...
                if smoothing == 1: 
                    ...
                    outputdir = join(homedir, 'miniblock', 'Outputs', runTypeIdx ,f'sm_{sm_fwhm}_vox_{subString}_{runTypeIdx}_run_{runNum}')
                
                else: 
                    data = interpData
                    outputdir = join(homedir, 'miniblock', 'Outputs', runTypeIdx , f'unsmoothed_{subString}_{runTypeIdx}_run_{runNum}')
                
                if upsampled_matrix.shape[0] != data.shape[-1]:
                    print("Design matrix and fMRI timepoints do not match!")
                    break
                    
                print(f'N = {data.shape[3]} TRs in this run\n')
                print(f'The dimensions of the data for each run are: {data.shape}\n')
                print(f'The stimulus duration is {stimdur} seconds\n')
                print(f'XYZ dimensionality is: {data.shape[:3]} (one slice only in this example)\n')
